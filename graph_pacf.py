...
pdf_loc = os.path.join("series", "pacf_graphs.pdf")
...
#    print(data)
    ...
    plot_pacf(np.array(data), lags=len(data)//2-1, ax=ax)
    plt.title("PACF: "+series_name)
    plt.xlabel("Lag")
    plt.ylabel("PACF")
#    plt.show()
    png_dir = os.path.join("series", series_name)
    os.makedirs(png_dir, exist_ok=True)
    png_path = os.path.join("series", series_name, "pacf.png")
